...
1. 语法错误，会直接提示，在 `exec` 之后会报错，整个事务放弃执行
2. 事务操作入队时，命令和操作的数据类型不匹配，但 Redis 没检测出错误，正常入队，等到 exec，只会对错误命令报错（部分命令已经完成执行修改）
3. 如果执行到一半发生了崩溃，可以通过 redis-check-aof（前提时开启了aof日志），清除已经完成的事务操作，数据库恢复后一致。

事务只支持（一致性、隔离性），原子性看aof开没开，而持久性就算是 always，因为是内存数据库的原因，还是可能存在丢失的可能。

在使用事务的时候，最好利用 pipeline(没有回滚机制，无法保证原子性)，一次性将多个命令打包给 Redis 服务器，一是减少了来回的网络IO 次数，
二是一次性发送过去，Redis 单线程执行，能保证事务的隔离性（也不一定，如果是 Redis Cluster，会hash到多个实例，最好配合 Lua 脚本）。
