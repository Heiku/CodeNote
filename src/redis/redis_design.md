...
缓存污染：一些数据访问次数很少，甚至只会访问一次，在访问后，一直留存在缓存中，占用内存（volatile-ttl）

在 LRU 的基础上，为每个数据增加一个计时器，统计数据的访问次数。当淘汰数据时，先根据访问次数进行筛选，将访问次数低的数据淘汰；如果次数相同， 那么比较两个数据的访问时效性，淘汰访问时间更久的数据。

Redis 在实现 LFU 策略的时候，将原来 24bit 的 lru 字段拆分成两部分：

1. ldt：lru 字段的前 16bit，表示数据的访问时间戳
2. counter：lru字段的后 8bit，表示数据的访问次数。（注意并不是每次访问+1，8bit最多255，Redis 优化了计数逻辑，使计数增长缓慢）
