...
* 数据量倾斜：实例上的数据分布不均衡，某个实例上的数据特别多

* 数据访问倾斜：每个集群上的实例数据量都差不多，但某个实例上的数据是热点数据，被访问得非常频繁。

1. bigKey 导致倾斜

某个实例上保存了 bigKey，导致这个实例上的数据量很大，内存消耗也会增加。所以我们在业务层生成数据的时候，要尽量避免把过多的数据保存在一个键值对上， 如果该 bigKey
是集合类型，那我们可以将其拆分成多个，分散在不同的实例上（比如可以根据用户uid划分实例）。

2. slot 分配不均衡导致倾斜

因为 Redis cluster slot 没有均衡分配，导致某些实例的 slot 占比会比较高（当然如果是性能好的机器，分配多点也合理）

3. Hash Tag 导致倾斜

Hash Tag 是指在键值对后面加上{}，客户端在计算 key 的 CRC16 值时，只会对 Hash Tag 进行计算（user:tag:{1212}）,也就是说：如果不同 key 的 Hash Tag
是一样的，那么这些数据将会被分配到同一个实例中（在 Redis Cluster 和 Codis 中用于支持事务操作和范围查询）

应对：如果是存在热点数据，可以在 key 上加上随机前缀，让这个数据分布在多个副本中（只读数据的情况，如果是读写数据，还得维持多个副本的一致性）

### Redis Cluster 规模

Redis Cluster 运行时，每个实例节点都会保存其他节点的 slot 和实例的对应关系（slot 关系映射表），以及自身的状态。

#### Gossip

1. 每个实例之间会按照一定的频率，从集群中挑选一些实例，PING 消息发送给实例，用来检查实例是否在线，并交换彼此的消息状态。PING 中包含了当前实例 的自身状态、部分其他实例的状态、以及 slot 映射表

2. 接收方在收到 PING 之后，会发送 PONG 消息，消息的格式和 PING 一样。

这样，随着 Gossip 运行一段时间，所有的实例节点都掌握了全部的节点信息和slot映射表。Gossip协议受 __通信消息大小__ 和 __通信频率__ 两方面影响。 所以，为了降低通信消息大小，Redis Cluster
集群的实例总量应该控制在 500 以内，这样可以有效避免实例间传输的数据过大，导致网络IO成本增高，性能下降。 同时，为了降低通信频率，我们可以调整实例的 `cluster-node-timeout 15`
，可以稍微调大一点，避免实例间频繁发送 PING 消息进行数据切换，但也不能 设置过大，过大会导致无法及时检测到节点下线，slot迁移受到影响，影响集群正常使用。

#### Codis

Codis 将 slot信息保存在第三方存储上（zookeeper），无论集群实例有多少，在zookeeper上只会存储一份，当新增 codis server 时，proxy可以直接从
zookeeper上获取全局的路由表，也就是说，新增节点并不会增加集群内的通信开销。

Redis Cluster 将所有功能实现在了 Redis实例上，包括路由表的交换、实例的健康检查、故障的自动切换等等，部署十分简单，多个实例组成集群即可对外提供访问。
但缺点是每个实例的工作比较中，如果不断扩展实例，集群内的通信性能也会下降。

Codis将功能拆分成多个组件：codis-proxy负责转发请求、codis-dashboard 负责路由表的转发、数据迁移控制，codis-server 负责数据存储和数据迁移， 哨兵负责故障自动切换。