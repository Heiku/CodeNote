...
为了确保 Class 文件符合当前虚拟机的要求，需要对其字节流数据进行验证，主要包括了格式验证、元数据验证、字节码验证和
符号引用验证。
...
初始化阶段是执行类构造器 <clint> 方法的过程，<clint> 方法由类变量赋值动作和静态语句块按照在源文件出现的顺序合并而成，
该合并动作由编译器完成
...
由于动态分派是非常频繁的操作，因此在虚拟机的实际实现钟，会基于性能的考虑，并不会如此频繁的搜索对应的方法，
一般会在方法区钟建立一个虚方法表，使用虚方法表代替方法查询以提高性能。
