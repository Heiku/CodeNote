...
在执行语句的过程中，产生的 redo 日志被划分成了若干个不可分割的 __组__（本质上是对 __底层页面的一次原子访问过程__ ），一个 `mtr` 可以包含一组 `redo` 日志，在崩溃恢复的时这一组 redo 日志作为一个不可分割的整体。比如以下几种情况都会被当作一个组（逻辑上是修改一个页，受关联影响会涉及多个页的修改，所以划分一组保证页修改的原子性）。
...
`checkpoint_lsn` ：代表当前系统可以被覆盖的 redo 日志总量是多少（即 redo 日志对应的脏页已经被刷入磁盘，已经不需要本次 redo 记录）



```
checkpoint_Lsn	...	flushed_to_disk_lsn	...	lsn
```
...
```
下面的内容都是 select ... in share model | for update（锁定读）
```



1. 存在幻读，所以不加 next-key lock、gap lock，所以都是行锁
2. 等值匹配（索引）的时候，会先判断条件，满足条件后再加锁
   * 不管聚簇索引还是二级索引，都会将关联的行进行加锁
3. 范围匹配的话，如果是 <=  指明上区间，那么将会额外扫描一行，先加锁判断条件解锁，注意区分索引类型
   * 如果是聚簇索引，那么将从下值开始扫描，加锁，到上区间+1行时，会加锁判断不满足条件再解锁（会临时锁住下一行）
   * 如果是二级索引，能利用到 `索引条件下推`，因为最后一行不满足所以直接返回 server 层，导致临时锁住的下一行没有解锁，即额外多锁住了一行（这一行不会关联锁聚簇索引行）
4. 如果不走索引（全表扫描），不管等不等值（毕竟都全表扫描了），会为每一行加锁，然后判断条件后，看看是否解锁



### READ_REPEATED

1. MySQL 中的 `READ_REPEATED` 保证了无法出现幻读，所以采用了 next-key lock 、gap lock
2. 主键等值匹配：
   * 存在对应的值：那么只锁住该行
   * 不存在，那么再该值对应的范围区间内加上 gap 锁，保证无法插入对应的值
3. 主键范围匹配：
   * <=，和上面一样，会加锁判断下一行，但这一行上的 `next-key lock` 并不会背释放，也就是 <= n + 1 都会加上 next-key lock
   * 如果关联二级索引，会给二级索引关联行加行锁
4. 唯一二级索引范围匹配
   * 二级索引范围上给各行加上 next-key lock，对于关联的主键行加上 `行锁`
   * `UPDATE hero SET country = '汉' WHERE name <= 'c曹操';` 二级索引，但是是 `update`，所以不能使用 `index Condition push`，因为要回表，所以也会锁住额外的主键行，但却不会释放
5. 普通索引等值匹配
   * 因为普通索引无法保证唯一，所以等值匹配后会对该行加上 next-key lock + gap lock ，反正该行左右范围无法插入
6. 全表扫描
   `READ_REPAETED` 下对于未建索引导致的全表扫描，会对所有行加上 next-key lock，但在判断之后不会释放（因为未建立索引，会导致下次插入的值可能出现在任何行的区间范围，所以不释放，导致全表锁住）