...
同步预读。对于第二次读请求，如果所读的页面不再 Cache 中，即不再预读的页中，表明文件访问不是顺序访问，系统继续采用同步预读。如果所读的页面在 Cache 中，
...
N叉数中，N 取决于数据页的大小。数据页太小会导致层数太深，太大会导致加载数据页到内存的时间会更长，在单页查询的时候也会
花费更多时间（尽管在页中已经有dict）

B+ 树中叶子节点存储的实际上是 Innodb 页，在页中存储多个真实的数据行（可以翻阅小册），一页大小一般为16K，基本上可以存储
8-16条真实数据行。

![](/img/mysql-index-tree.png)
![](/img/mysql-innodb-leaf-page-struct.png)

在每一个数据页中，会存在多条数据行，所有数据行都是有序的，再插入的数据页的时候，将4-8条数据行分为一组（槽），记录下槽的id，
存在有序数组中，最后在查找的时候通过有序数组二分查找对应的槽，再进行顺序遍历操作的链表（O（logN）+ O(1)）
...
因为 Mysql 的存储结构使用的是 B+ 树，当指定关键字下的叶子节点无法容纳更多的数据时，将产生页分裂，新增关键字和关键字下的叶子
节点。

#### 50% 分裂

在新增关键字后，将之前关键字下的 50% 数据移至新的叶子节点下。优势在于如果在随机插入的情况下，触发分裂的时间更晚，但缺点在于
空间利用率不高，新旧关键字下的叶子节点只有 50% 的剩余存储空间，如果时递增插入时，将很容易（更快）触发下一次页分裂。

#### 100% 分裂 （优化）

直接新增关键字和叶子节点，不迁移旧关键字数据，这样保证了页面的利用率仍然是 100%，优点是不需要迁移数据，同时当插入的数据
是自增时，将会填满空间后再触发页分裂，分裂的概率降低了。但缺点是如果数据是随机的，在插入原页面的时候，将频繁触发页分类操作。

可以采用 `alter table T drop(add) index(k)` 重建索引，避免额外页空间对磁盘空间的占用。

### 分析

根据B+ 树的定义，每个节点内部都会有多个子节点(关键字)。对于Mysql，每一个节点就是一个数据块(和页大小相同),每个节点只需要一次 I/O 就可以完全载入
...
如果是哈希表，在单查询地时候虽然能获得 O(1) 的速度，当在范围查询或者排序的时候，因为哈希表的特点，数据分布在多个桶中，
所以在查询的时候就需要全表查询，这在数据量非常大的情况性能很差。
...
在 InnoDB 中，事务得 ACID 主要是通过 InnoDB 日志和锁保证得。事务的隔离性是通过数据库锁机制实现，持久性通过 redo log 实现，
而原子性和一致性通过 undo log 实现。

undo log: 在操作任何数据之前，首先将数据备份到一个地方，然后进行数据的修改，如果出现错误或者用户执行 ROLLBACK，系统可以
利用 undo log 中的备份将数据恢复到事务开始之前的状态。

redo log: redo log 中记录的是新数据的备份，在事务提交之前，只要将 redo log 持久化即可，不需要将数据持久化。当系统奔溃时，
虽然数据没有持久化，但 redo log已经持久化，系统可以根据 redo log的内容，将数据恢复到最新的状态。 
